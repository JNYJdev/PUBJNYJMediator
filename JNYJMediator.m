...
NSString * const kJNYJMediatorParamsKeySwiftTargetModuleName = @"kJNYJMediatorParamsKeySwiftTargetModuleName";

@interface JNYJMediator ()

@property (nonatomic, strong) NSMutableDictionary *cachedTarget;

@end

@implementation JNYJMediator

+(void)load{
    //NSLog(@"---STARTING-----JNYJMediator");
}
#pragma mark - public methods
+ (instancetype)sharedInstance
{
    static JNYJMediator *mediator;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        mediator = [[JNYJMediator alloc] init];
  ...
  shouldCacheTarget:(BOOL)shouldCacheTarget
{
    ...
    NSString *swiftModuleName = params[kJNYJMediatorParamsKeySwiftTargetModuleName];
    ...
    if (target == nil) {
        // 这里是处理无响应请求的地方之一，这个demo做得比较简单，
        //如果没有可以响应的target，就直接return了。实际开发过程中是可以事先给一个固定的target专门用于在这个时候顶上，
        //然后处理这种请求的
...
{
    /*
     fullTargetName在oc环境下，就是Target_XXXX。要带上Target_前缀。
     在swift环境下，就是XXXModule.Target_YYY。不光要带上Target_前缀，还要带上模块名。
     */
...
- (void)NoTargetActionResponseWithTargetString:(NSString *)targetString
                                selectorString:(NSString *)selectorString originParams:(NSDictionary *)originParams
{
...
JNYJMediator* _Nonnull CT(void){
    return [JNYJMediator sharedInstance];
};
